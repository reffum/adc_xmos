...
//
// These variables are created to determine the actual bitrate of the audio stream.
// SamplesOr and SamplesOrCounter is initialized to 0 at beginneng of each stream, after AudioHwConfig().
// Each USB samples is OR-ed with SamplesOr and SamplesOrCounter is incremented. When SamplesOrCounter equal
// SamplesOrNum, call AudioHwConfigBitrate() and pass them SamplesOr.
// User code determine actual bitrate by number of MSB zeros in SamplesOr.
//
static unsigned SamplesOr = 0;
int SamplesOrCounter = 0;
const int SamplesOrNum = 10000;
...
#pragma loop unroll
        for(int i = 0; i < NUM_USB_CHAN_OUT; i++)
        {
            ...
            // This code ored received samples for determine actual bitrate
            if(dsdMode == DSD_MODE_OFF)
            {
                SamplesOr |= samplesOut[i];
                SamplesOrCounter++;

                if(SamplesOrCounter == (SamplesOrNum - 1))
                {
                    AudioHwConfigBitrate(SamplesOr);
                    SamplesOrCounter = SamplesOr = 0;
                }
            }
        }
#else
...
)
{
    ...
    while(1)
    {
...

        {
            ...
            // Reset variable for determine acutal bitrate
            SamplesOr = SamplesOrCounter = 0;
        }
